...
#%%
#load information where country is on our map
...
#%%
"""GET AND SCALE THE EXPLANATORY VARIABLES FOR COUNTRY"""
...
# %%
crops,counts=np.unique(lucas_with_covariates[lucas_with_covariates["nuts0"]==country]["DGPCM_code"],return_counts=True)
...
np.save(prior_proba_output_dir+country+"_"+str(year)+"_considered_crops.npy",considered_crops_country)
for c,capri_code in enumerate(capri_codes):
    ...
    np.save(prior_proba_output_dir+capri_code+"_"+str(year)+".npy",all_probas)
